...
SECRET_SALT="${1:-EHPT02_SECRET_SALT_2026}"

# EHPT_02 repo goes to /opt/lab02-setup/ (root:root 700 — invisible to student)
EHPT_DIR="/opt/lab02-setup"

echo "=============================================================================="
echo "[*] Preparing Lab 02 Black-Box Template VM"
echo "    Theme: Cryptographic Failures | Security Misconfiguration | Vulnerable Components"
echo "=============================================================================="
...
# ---- 3. Write the secret salt to /etc/lab02.conf (root:root 600 — student CANNOT read) ----
echo "[+] Step 3: Storing secret salt in /etc/lab02.conf (root-only)..."
echo "${SECRET_SALT}" > /etc/lab02.conf
chown root:root /etc/lab02.conf
chmod 600 /etc/lab02.conf
echo "    Salt stored at /etc/lab02.conf  (mode: 600 — student access: DENIED)"

# ---- 4. Clone EHPT_02 repo to /opt/lab02-setup/ (outside student home, root-only) ----
echo "[+] Step 4: Deploying EHPT_02 repo to ${EHPT_DIR} (root-only, invisible to student)..."
if [ ! -d "${EHPT_DIR}/.git" ]; then
    git clone https://github.com/Abdelrhman-Mohamedd/EHPT_02.git "$EHPT_DIR"
else
    git -C "$EHPT_DIR" pull
fi
...
cat > "${AUTOSTART_DIR}/lab02-setup.desktop" << 'DESKTOP'
[Desktop Entry]
Type=Application
Name=Lab 02 First Boot Setup
...
cat << EOF > /etc/sudoers.d/lab02-setup
# Lab 02 Restricted sudo: student may ONLY run setup.sh as root
# setup.sh lives in /opt/lab02-setup/ (root:root 500) — student cannot read or modify it.
student ALL=(root) NOPASSWD: /opt/lab02-setup/setup.sh *
EOF
chmod 440 /etc/sudoers.d/lab02-setup
visudo -c -f /etc/sudoers.d/lab02-setup && echo "    Sudoers rule OK at /etc/sudoers.d/lab02-setup" || echo "[!] sudoers syntax error!"
...
╔═══════════════════════════════════════════════════════════════════╗
║       VaultTech Ethical Hacking Lab 02 — Black-Box Appliance      ║
...
echo "     EHPT_02 Repo    : /opt/lab02-setup/  (root:root 700 — student cannot see)"
echo "     Secret Salt     : /etc/lab02.conf    (root:root 600 — student cannot read)"
echo "     Student Sudo    : ONLY /opt/lab02-setup/setup.sh (nothing else)"
